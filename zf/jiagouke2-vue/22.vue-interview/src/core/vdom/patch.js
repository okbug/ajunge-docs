...
export function createPatchFunction (backend) { // 这里定义好所有的钩子,在创建元素时调用钩子。  
  let i, j
  const cbs = {}
  ...
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    ...
    if (isDef(i)) { // 是否是重新激活
      ...
      // 调用组件的init方法 // Vue.extend().$mount
      if (isDef(i = i.hook) && isDef(i = i.init)) {
        i(vnode, false /* hydrating */) // 组件的实例上就有了$el
      }
  ...
  function initComponent (vnode, insertedVnodeQueue) {
    ...
    vnode.elm = vnode.componentInstance.$el // 把组件创建好的元素放到vnode.elm上
  ...
  function createChildren (vnode, children, insertedVnodeQueue) {
    if (Array.isArray(children)) {
      ...
      for (let i = 0; i < children.length; ++i) { // 递归创建子节点
  ...
  function patchVnode ( // 比较虚拟节点
    oldVnode, // 老的虚拟节点   vnode.el
    vnode, // 新的虚拟节点
    insertedVnodeQueue,
    ownerArray,
    index,
    ...
    let i
    const data = vnode.data // 如果是组件调用组件的prepatch钩子
    if (isDef(data) && isDef(i = data.hook) && isDef(i = i.prepatch)) {
      i(oldVnode, vnode)
    }

    const oldCh = oldVnode.children // 比较两个孩子节点
    const ch = vnode.children
    if (isDef(data) && isPatchable(vnode)) { // 更新当前节点
