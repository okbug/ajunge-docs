...
class Complication {
    ...
    build(callback) {
        ...
        for(let entryName in entry){
            ...
            let chunk = {name:entryName,entryModule,modules:this.modules.filter(item=>{
        ...
        traverse(ast,{
            CallExpression:({node})=>{
                if(node.callee.name === 'require'){
                    ...
                    let alreadyImportedModuleIds = this.modules.map(item=>item.id)
                    //依赖的模块绝对路径放到当前的模块的依赖数组里
                    //if(!alreadyImportedModuleIds.includes(depModuleId))
